...
## 异常信息说明
如果在爬取过程中遇到 `[网络异常] 请求一级评论页超时或失败` 或者 `抓取楼中楼超时或失败`：
- 这是因为 B站对接口访问频率有严格限制，或是由于你所在网络环境波动。
- 脚本已内置防崩溃与重连机制（缩减了冗长的底层报错堆栈），对于一级评论会自动等待并重试；对于由于严苛的反爬机制偶尔漏掉获取的某些二级回复，脚本会记录简化日志并跳过，不影响整体主流程，这属于正常的爬虫现象。
